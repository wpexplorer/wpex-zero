...
class cmb_Meta_Box_types {
	...
	private static function repeat_table_close() {
		echo '</tbody></table><p class="add-row"><a data-selector="', cmb_Meta_Box::$field['id'] ,'_repeat" class="add-row-button button" href="#">'. __( 'Add Row', 'wpex-zero' ) .'</a></p>';
	}
	...
	private static function repeat_cell( $input ) {
		return '<td>'. $input .'</td><td class="remove-row"><a class="button remove-row-button" href="#">'. __( 'Remove', 'wpex-zero' ) .'</a></td>';
	}
	...
	public static function file_list( $field, $meta, $object_id ) {
		...
		echo '<input class="cmb_upload_button button cmb_upload_list" type="button" value="'. __( 'Add or Upload File', 'wpex-zero' ) .'" />', self::desc( true );
			...
			foreach ( $meta as $id => $fullurl ) {
				if ( self::is_valid_img_ext( $fullurl ) ) {
					...
					'<li class="img_status">',
						...
						'<p><a href="#" class="cmb_remove_file_button">'. __( 'Remove Image', 'wpex-zero' ) .'</a></p>
					...
					echo
					'<li>',
						__( 'File:', 'wpex-zero' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $fullurl, '" target="_blank" rel="external">'. __( 'Download', 'wpex-zero' ) .'</a> / <a href="#" class="cmb_remove_file_button">'. __( 'Remove', 'wpex-zero' ) .'</a>)
						<input type="hidden" id="filelist-', $id ,'" name="', $field['id'] ,'[', $id ,']" value="', $fullurl ,'" />
					</li>';
				}
			}
	...
	public static function file( $field, $meta, $object_id, $object_type ) {
		...
		echo '<input class="cmb_upload_button button" type="button" value="'. __( 'Add or Upload File', 'wpex-zero' ) .'" />';
		...
		'<div id="', $field['id'], '_status" class="cmb_media_status">';
			if ( ! empty( $meta ) ) {
				...
				if ( self::is_valid_img_ext( $meta ) ) {
					...
					echo '<p><a href="#" class="cmb_remove_file_button" rel="', $field['id'], '">'. __( 'Remove Image', 'wpex-zero' ) .'</a></p>';
					echo '</div>';
				} else {
					...
					echo __( 'File:', 'wpex-zero' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $meta, '" target="_blank" rel="external">'. __( 'Download', 'wpex-zero' ) .'</a> / <a href="#" class="cmb_remove_file_button" rel="', $field['id'], '">'. __( 'Remove', 'wpex-zero' ) .'</a>)';
				}
			}
		echo '</div>';
	}
